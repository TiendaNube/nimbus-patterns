...
- Added `open`, `onRemove`, `snapPoints`, `defaultSnap`, `children`, `closeOnOutsidePress`, `ignoreAttributeName`, `needRemoveScroll`, `grabberLabel`, `zIndex`, and `root` properties to the component API. ([#177](https://github.com/TiendaNube/nimbus-patterns/pull/177) by [@jffs](https://github.com/jffs))
...
- Widened the grabber's touch target to Nimbus's 44px minimum for mobile drag handles, without enlarging its visible pill or shifting `BottomSheet.Header`. ([#177](https://github.com/TiendaNube/nimbus-patterns/pull/177) by [@jffs](https://github.com/jffs))
