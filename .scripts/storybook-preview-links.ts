 ...
 * sanitizer collapses dots, slashes and repeated separators (`Patterns/
 * PlanDisplay/PlanDisplay.Card` becomes `patterns-plandisplay-plandisplay-card`),
 * so any hand-rolled slug eventually produces a dead link.
 *
 ...
 *
 * Ported from nimbus-design-system's script of the same name — kept in sync
 * by hand, since each repo's component layout differs just enough (this repo
 * has no separate styles package) that sharing the file isn't worth it.
 */
...
const COMPONENT_ROOT_PATTERN = /^packages\/react\/src\/components\/[^/]+/;
...
const resolveStoriesFile = (
  ...
  const componentRoot = COMPONENT_ROOT_PATTERN.exec(changedFile)?.[0];
